            ...
            <tr key={index}>
              ...
              <td>
                <div className='fs-5'>
                  ...
                  <Link className='Link' to={repo.name}>{repo.name}</Link>
                </div>
                <div>{repo.description}</div>
                <div>
                  {repo.topics.map((topic, index) =>
                    <span className="badge rounded-pill me-1" key={index}>{topic}</span>
                  )}
                </div>
              </td>
...
class BackToHome extends Component {
  render() {
    return (
      <Link className="btn btn-outline-secondary btnBack mb-3" to='/' role='button'>
...
class LoadMore extends Component {
  render() {
    if (10 * this.props.page < this.props.publicRepoNum) {
      ...
      return (
        <tr className="tfoot">
          <th className='text-center text-white' colSpan="3">No more repository</th>
        </tr>
      );
    }
  }
